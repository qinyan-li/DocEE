...
class LSTMMTL2CompleteGraphModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        super(LSTMMTL2CompleteGraphModel, self).__init__()
        ...
        if ner_model is None:
            self.ner_model = LSTMCRFNERModel(config)
        else:
    ...
    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_arg_rel_info):
        ...
        # return self.config.combination_loss_weight * sum(arg_combination_loss) \
        #     + self.config.role_loss_weight * sum(arg_role_loss)
    ...
    def get_mix_loss(
        ...
        if not self.dynamic_loss:
            ...
            '''
            return loss_batch_avg * (
                sum(doc_ner_loss_list)/self.lambda_1**2   +  sum(doc_event_loss_list) + torch.log(self.lambda_1)
            ) #+ self.lambda_1**2
    ...
    def get_eval_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_arg_rel_info):
        ...
        if not self.config.event_relevant_combination:
            ...
            if len(raw_combinations) < 1:
                raw_combinations = bron_kerbosch_pivoting_decode(pred_adj_mat, 2)
    ...
    def reveal_event_arg_obj(self, combination, roles, num_fields):
        ...
        for span_idx, roles in results:
            if roles is not None:
                if isinstance(roles, list):
                    for role in roles:
                        ret_results[role] = span_idx
                else:
                    ret_results[roles] = span_idx
        return ret_results
    ...
    def pred_adj_mat_reorgnise(self, pred_adj_mat):
        ...
        adj_mat = torch.bitwise_and(pred_adj_mat, pred_adj_mat.T)
