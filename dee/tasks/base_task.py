...
class TaskSetting(object):
    ...
    def __init__(self, key_attrs, attr_default_pairs, **kwargs):
        for key_attr in TaskSetting.base_key_attrs:
            setattr(self, key_attr, kwargs.get(key_attr))
    ...
    @classmethod
    def from_pretrained(cls, config_filepath: str, **kwargs):
        config = cls(**kwargs)
        config_dict = default_load_json(config_filepath)
...
class BasePytorchTask(object):
    ...
    def __init__(self, setting, only_master_logging=False):
        ...
        if self.in_distributed_mode() and not dist.is_initialized():
            ...
            self.logging(
                "World Size {} Rank {}, Local Rank {}, Device Num {}, Device {}".format(
                    dist.get_world_size(),
                    dist.get_rank(),
                    self.setting.local_rank,
    ...
    def _check_setting_validity(self):
        ...
        self.setting.train_batch_size = int(
            self.setting.train_batch_size / self.setting.gradient_accumulation_steps
        )

        # check output dir
        if os.path.exists(self.setting.output_dir) and os.listdir(
            self.setting.output_dir
        ):
            self.logging(
        ...
        # check model dir
        if os.path.exists(self.setting.model_dir) and os.listdir(
            self.setting.model_dir
        ):
            self.logging(
                "Model directory ({}) already exists and is not empty.".format(
                    self.setting.model_dir
                ),
                level=logging.WARNING,
            )
    ...
    def _init_device(self):
        ...
        if self.setting.local_rank == -1 or self.setting.no_cuda:
            self.device = torch.device(
                "cuda"
                if torch.cuda.is_available() and not self.setting.no_cuda
                else "cpu"
            )
            ...
            if self.setting.fp16:
                ...
                self.setting.fp16 = (
                    False  # (see https://github.com/pytorch/pytorch/pull/13496)
                )
        self.logging(
    ...
    def load_example_feature_dataset(
        self,
        load_example_func,
    ...
    def get_current_train_batch_size(self):
        if self.in_distributed_mode():
            train_batch_size = max(
                self.setting.train_batch_size // dist.get_world_size(), 1
            )
        else:
    ...
    def base_train(
        ...
        for epoch_idx in trange(
            base_epoch_idx,
            int(self.setting.num_train_epochs),
            desc="Epoch",
            ncols=80,
            ascii=True,
        ):
            ...
            if self.only_master_logging and not self.is_master_node():
                step_batch_iter = enumerate(pbar)
            else:
                pbar = tqdm(train_dataloader, desc=iter_desc, ascii=True)
                step_batch_iter = enumerate(pbar)
            ...
            for step, batch in step_batch_iter:
                ...
                if isinstance(total_loss, dict):
                    ...
                    # elif 10 <= epoch_idx < 15:
                    #     loss = total_loss['event_cls']
                    # elif 15 <= epoch_idx < 25:
                    # loss = total_loss['arg_combination_loss']
                    # elif 25 <= epoch_idx < 30:
                    #     loss = total_loss['arg_role_loss']
                    # else:
                ...
                # backward
                loss.backward()
                if self.setting.global_grad_clip:
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), 5.0)
                elif self.setting.biaffine_grad_clip:
                    torch.nn.utils.clip_grad_norm_(
                        self.model.biaffine.parameters(), 5.0
                    )
                ...
                nb_tr_examples += (
                    self.setting.train_batch_size
                )  # may not be very accurate due to incomplete batch
                nb_tr_steps += 1
                if (step + 1) % self.setting.gradient_accumulation_steps == 0:
                    if self.setting.fp16 or self.setting.optimize_on_cpu:
                        if self.setting.fp16 and self.setting.loss_scale != 1.0:
                            ...
                            for param in self.model.parameters():
                                param.grad.data = (
                                    param.grad.data / self.setting.loss_scale
                                )
                    ...
                    if self.is_master_node():
                        if isinstance(total_loss, dict):
                            ...
                            pbar.set_postfix({"loss": total_loss["loss"].item()})
                        else:
                            self.summary_writer.add_scalar(
                                "Loss", total_loss.item(), global_step=global_step
                            )
    ...
    def base_eval(
        ...
        **func_kwargs,
        ...
        self.logging("=" * 20 + "Start Base Evaluation: " + dump_pkl_path + "=" * 20)
        ...
        if reduce_info_type.lower() == "sum":
            ...
            raise Exception(
                "Unsupported reduce metric type {}".format(reduce_info_type)
            )
    ...
    def save_checkpoint(self, cpt_file_name=None, epoch=None):
        ...
        if self.model:
            if isinstance(self.model, para.DataParallel) or isinstance(
                self.model, para.DistributedDataParallel
            ):
    ...
    def resume_checkpoint(
        ...
        if cpt_file_path is None:  # use provided path with highest priority
            if (
                cpt_file_name is None
            ):  # no path and no name will resort to the default cpt name
                cpt_file_name = self.setting.cpt_file_name
            cpt_file_path = os.path.join(self.setting.model_dir, cpt_file_name)
        ...
        self.best_f1 = store_dict["best_f1"]
        self.logging(f"Resume best_f1: {100 * self.best_f1:.3f}")
        ...
        if resume_optimizer:
            if self.optimizer and "optimizer_state" in store_dict:
                ...
                raise Exception(
                    "Resume optimizer failed, dict.keys = {}".format(store_dict.keys())
                )
        else:
