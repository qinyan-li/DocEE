...
class SelfAttention(nn.Module):
    ...
    def forward(self, hidden, mask=None, return_scores=False):
        if mask is not None:
...
def dot_attention(hidden1, hidden2):
    d_k = hidden1.size(-1)
    scores = torch.matmul(hidden1, hidden2.transpose(-2, -1)) / math.sqrt(d_k)
    return scores


class MultiHeadedDotAttention(nn.Module):
    def __init__(self, h, d_model, aggregation="max", dropout=0.1):
        """Take in model size and number of heads."""
    ...
    def forward(self, hidden):
        nbatches = hidden.size(0)

        # 1) Do all the linear projections in batch from d_model => h x d_k
        query, key = [
            linear_layer(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
            for linear_layer, x in zip(self.linears, (hidden, hidden))
        ]
