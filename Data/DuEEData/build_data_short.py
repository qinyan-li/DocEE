...
def build(
    ...
    for d in load_line_json_iterator(filepath):
        ...
        # sents = reorganise_sents(sents, max_seq_len, concat=True) # qy: 不合并短句
...
if __name__ == "__main__":
    ...
    build(
        template.event_type2event_class, "train.json", "luge_train_without_trigger_short.json"
    )
    build(template.event_type2event_class, "dev.json", "luge_dev_without_trigger_short.json")
    build(
        template.event_type2event_class,
        "sample.json",
        "luge_sample_without_trigger_short.json",
    )
    build(
        template.event_type2event_class,
        "submit_test.json",
        "luge_submit_without_trigger_short.json",
        inference=True,
    )
    '''
