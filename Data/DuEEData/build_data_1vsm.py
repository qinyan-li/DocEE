        ...
        if not inference:
            ...
            for event_idx, ins in enumerate(d["event_list"]):
                ...
                for arg_pair in ins["arguments"]:
                    ...
                    #role2arg[arg_pair["role"]] = arg_pair["argument"] # qy: 当前每个role只取了一个argument
                    ann_valid_mspans.append(arg_pair["argument"]) # qy: all arguments
                    ann_mspan2guess_field[arg_pair["argument"]] = arg_pair["role"] # qy: 每个argument只取了一个role？
...
if __name__ == "__main__":
    ...
    build(
        template.event_type2event_class, "train.json", "luge_train_without_trigger_multirole.json"
    )
    build(template.event_type2event_class, "dev.json", "luge_dev_without_trigger_multirole.json")
    build(
        template.event_type2event_class,
        "sample.json",
        "luge_sample_without_trigger_multirole.json",
    )
    build(
        template.event_type2event_class,
        "submit_test.json",
        "luge_submit_without_trigger_multirole.json",
        inference=True,
    )
    '''
